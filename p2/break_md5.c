...
#define PASS_LEN 6
//#define NUM_THREADS  //numero de nucleos
// \r vuelve al principio de la linea en la que estamos (barra de estado)
...
char *break_pass(char *md5) {
    ...
    for(long i=0; i < bound; i++) { //dividir este bucle
    //i%NUM_THREADS == num_threads
        long_to_pass(i, pass);
